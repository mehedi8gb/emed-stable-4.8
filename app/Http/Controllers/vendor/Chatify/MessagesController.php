...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch,
            'user_avatar' => asset('public/'.config('chatify.user_avatar.folder') . '/' . $fetch->avatar),
        ]);
    }
    ...
    {
        $path = 'public/' . config('chatify.attachments.folder') . '/' . $fileName;
        if (file_exists($path)) {
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        for ($i = 0; $i < count($shared); $i++) {
            $sharedPhotos .= view('Chatify::layouts.listItem', [
                'get' => 'sharedPhoto',
                'images' => asset('public/attachments/' . $shared[$i]),
            ])->render();
        }
        // send the response
        return Response::json([
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (Auth::user()->avatar != config('chatify.user_avatar.default')) {
                        $path = storage_path('public/' . config('chatify.user_avatar.folder') . '/' . Auth::user()->avatar);
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }
