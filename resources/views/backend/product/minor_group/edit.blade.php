    ...
    <div class="col-lg-8 mx-auto">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">Edit {!!'<mark>'.$minor_group->name.'`s</mark> '.translate('Dosage Form')!!}</h5>
            </div>
            <div class="card-body">
                <form class="form-horizontal" action="{{ route('minor.group.update',$minor_group->id) }}" method="POST" enctype="multipart/form-data">
                	@csrf
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label">{{translate('Name')}}</label>
                        <div class="col-md-9">
                            <input type="text"  id="name" name="name" value="{{$minor_group->name}}" class="form-control">
                        </div>
                    </div>
                    {{-- <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <select class="select2 form-control aiz-selectpicker" name="parent_id" data-toggle="select2" data-placeholder="Choose ..." data-live-search="true">
                                ...
                                @foreach ($minor_group as $category)
                                    ...
                                    @foreach ($category->childrenminor_group as $childCategory)
                                        @include('minor_group.child_category', ['child_category' => $childCategory])
                                    @endforeach
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <select name="type" required class="form-control aiz-selectpicker mb-2 mb-md-0">
                                <option {{ $minor_group->type == 0 ? 'selected':'' }}>{{translate('Physical')}}</option>
                                <option {{ $minor_group->type == 1 ? 'selected':'' }}>{{translate('Digital')}}</option>
                            </select>
                        </div>
                    </div>
                    {{-- <div class="form-group row">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <div class="input-group" data-toggle="aizuploader" data-type="image">
                                ...
                                <div class="form-control file-amount">{{ translate('Choose File') }}</div>
                                <input type="hidden" name="icon" value="{{$minor_group->icon}}" class="selected-files">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <input type="text" class="form-control" name="slug" value="{{$minor_group->slug}}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label">{{translate('Meta Title')}}</label>
                        <div class="col-md-9">
                            <input type="text" class="form-control" name="meta" value="{{$minor_group->meta}}">
                        </div>
                    </div>
