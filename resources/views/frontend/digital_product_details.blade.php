                    ...
                    <div class="col-xl-7 col-lg-6">
                        <div class="text-left">
                            <h1 class="mb-2 fs-20 fw-600">
                              <small>{{translate('Test Name')}}: </small> {{ $detailedProduct->getTranslation('name') }}
                            </h1>
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-6 text-right">
                                    {{-- @php
                                        ...
                                        //}
                                    @endphp --}}
                                    <span class="badge badge-md badge-inline badge-pill badge-success">{{ translate('Safe')}}</span>
                                </div>
                            </div>
                            ...
                            <div class="row align-items-center">
                                <div class="col-auto">
                                    <small class="mr-2 opacity-90">{{ translate('Sample Collection Type')}}: </small><br>
                                    <span class="fw-600">{{ $detailedProduct->category->name }}</span>
                                </div>
                                @if (get_setting('conversation_system') == 1)
                                    <div class="col-auto">
                                        <button class="btn btn-sm btn-success" onclick="show_chat_modal()">{{ translate('Ask About Test')}}</button>
                                    </div>
                                @endif
                            ...
                            <div class="row no-gutters mt-3">
                                <div class="col-auto mr-2">
                                    ...
                                    <div class="fw-600 opacity-100">

                                        <!-- Button trigger modal -->
<button type="button" class="btn btn-md-block btn-primary" data-toggle="modal" data-target="#exampleModalLong">
    {{ translate('Select Lab')}}
  </button>
...
<div class="modal fade" id="exampleModalLong" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
 ...
 <input type="radio" class="btn-check" name="options" id="option{{$item->user_id}}" autocomplete="off">
 <label class="btn btn-soft-primary" for="option{{$item->user_id}}">{{ $item->name }}</label>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary">Save changes</button>
                            ...
                            @if(home_price($detailedProduct) != home_discounted_price($detailedProduct))
                                ...
                                <div class="row no-gutters mt-3">
                                    <div class="col-2">
                                        <div class="opacity-50 mt-2">{{ translate('Cost')}}:</div>
                                    </div>
                                    <div class="col-10">
                            ...
                            @else
                                <div class="row no-gutters mt-3">
                                    <div class="col-1">
                                        <div class="opacity-70">{{ translate('Cost')}}:</div>
                                    </div>
                                    <div class="col-11">
                                        <div class="">
                                            <strong class="h5 fw-500 text-primary">
                                                {{ home_discounted_price($detailedProduct) }}
                                            </strong>
                                            @if($detailedProduct->unit != null)
                                                <span class="opacity-70">/{{ $detailedProduct->getTranslation('unit') }}</span>
                            ...
                            <div class="mt-3">
                                ...
                                <button type="button" class="btn btn-primary buy-now fw-600" onclick="buyNow()">
                                    <i class="fad fa-vial"></i> {{ translate('Book Now')}}
                                </button>
                            </div>
                            ...
                            <div class="row no-gutters mt-4">
                                <div class="col-2">
                                    <div class="opacity-50 mt-2">{{ translate('Sample Type')}}:</div>
                                </div>
                                <div class="col-10">
                                    <div class=""> Blood</div>
                                </div>
                ...
                <div class="bg-white rounded shadow-sm">
                    <div class="border-bottom p-3">
                        <h3 class="fs-16 fw-600 mb-0 text-left">
                            <span class="">{{ translate('Related Tests')}}</span>
                        </h3>
                    </div>
                    <div class="p-3">
                        <div class="aiz-carousel gutters-5 half-outside-arrow" data-items="5" data-xl-items="3" data-lg-items="4"  data-md-items="3" data-sm-items="2" data-xs-items="2" data-arrows='true' data-infinite='true'>
                            @foreach (filter_products(\App\Product::where('category_id', $detailedProduct->category_id)->where('id', '!=', $detailedProduct->id)->where('digital', $detailedProduct->digital))->limit(10)->get() as $key => $related_product)
 ...
 <div class="bg-white shadow-sm mb-3">
                        <div class="position-relative p-3 text-left">
                            ...
                            @endif
                            <div class="opacity-50 fs-12 border-bottom">{{ translate('Service Provided By')}}</div>
                            @if($detailedProduct->added_by == 'seller' && get_setting('vendor_system_activation') == 1)
                                <a href="{{ route('lab.visit', $detailedProduct->user->shop->slug) }}" class="text-reset d-block fw-600">{{ $detailedProduct->user->shop->name }}
                                @if ($detailedProduct->user->seller->verification_status == 1)
        ...
        <div class="bg-white rounded shadow-sm mb-3">
            <div class="p-3 border-bottom fs-16 fw-600">
                {{ translate('Top Lab Tests')}}
            </div>
            <div class="p-3">
                <ul class="list-group list-group-flush">
                    @foreach (filter_products(\App\Product::where('user_id', $detailedProduct->user_id)->where('digital', $detailedProduct->digital)->orderBy('num_of_sale', 'desc'))->limit(6)->get() as $key => $top_product)
    ...
    <div class="modal fade" id="chat_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-zoom product-modal" id="modal-size" role="document">
            <div class="modal-content position-relative">
                ...
                <form class="" action="{{ route('send.message.to') }}" method="POST" enctype="multipart/form-data">
                    @csrf
