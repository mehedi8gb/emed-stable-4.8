...
(function($) {
    ...
    AIZ.extra = {
        ...
        // Product page quantity Calculation
        plusMinus: function() {
            ...
            $('.aiz-plus-minus button').on('click', function(e) {
                ...
                var inputQty = $("input[name='qtys']");
                var Qty = parseInt(inputQty.val());

                var currentVal = parseInt(input.val());
                var qtys = $("input[name='qtys']");
                var qtysVal = parseInt(qtys.val());


                if (!isNaN(currentVal)) {
                    if (type == "minus") {
                        if (currentVal > input.attr("min")) {
                            input.val(qtysVal - 1).change();
                            inputQty.val(qtysVal - 1).change();
                            qtys.val(qtysVal - 1).change();
                        }
            ...
            // Product page Strip quantity Calculation

            $('.strip-plus-minus input').each(function() {
                var $this = $(this);
                var min = parseInt($(this).attr("min"));
                var max = parseInt($(this).attr("max"));
                var value = parseInt($(this).val());
                ...
                if (value <= min) {
                    $this.siblings('[data-type="Sminus"]').attr('disabled', true)
                }
                if (value >= max) {
                    $this.siblings('[data-type="Splus"]').attr('disabled', true)
                }
            });
            $('.strip-plus-minus button').on('click', function(e) {
                e.preventDefault();

                var fieldName = $(this).attr("data-field");
                var type = $(this).attr("data-type");
                var input = $("input[name='" + fieldName + "']");
                var strips = $("input[name='strips']");
                var stripsVal = parseInt(strips.val());
                var currentVal = parseInt(input.val());
                var stripinput = $("input[name='strip_size']");
                var stripVal = parseInt(stripinput.val());

                if (!isNaN(currentVal)) {
                    if (type == "Sminus") {
                        if (currentVal > input.attr("min")) {
                            input.val((currentVal - stripVal)).change();
                            strips.val((stripsVal - 1)).change();
                        }
                        if (parseInt(input.val()) == input.attr("min")) {
                            $(this).attr("disabled", true);
                        }
            ...
            $('.strip-plus-minus .input').on('change', function() {
                var minValue = parseInt($(this).attr("min"));
                var maxValue = parseInt($(this).attr("max"));
                var valueCurrent = parseInt($(this).val());
                ...
                name = $(this).attr("name");
                if (valueCurrent >= minValue) {
                    $(this).siblings("[data-type='Sminus']").removeAttr("disabled");
                } else {
                    alert("Sorry, the minimum limit has been reached");
                    $(this).val($(this).data("oldValue"));
                }
                if (valueCurrent <= maxValue) {
                    $(this).siblings("[data-type='Splus']").removeAttr("disabled");
                } else {
                    alert("Sorry, the maximum limit has been reached");
                    $(this).val($(this).data("oldValue"));
                }
            });
            // Product page Strip quantity Calculation end



            // Product page Box quantity Calculation

            $('.box-plus-minus input').each(function() {
                var $this = $(this);
                var min = parseInt($(this).attr("min"));
                var max = parseInt($(this).attr("max"));
                var value = parseInt($(this).val());
                ...
                if (value <= min) {
                    $this.siblings('[data-type="Bminus"]').attr('disabled', true)
                }
                if (value >= max) {
                    $this.siblings('[data-type="Bplus"]').attr('disabled', true)
                }
            });
            $('.box-plus-minus button').on('click', function(e) {
                e.preventDefault();

                var fieldName = $(this).attr("data-field");
                var type = $(this).attr("data-type");
                var input = $("input[name='" + fieldName + "']");
                var strips = $("input[name='boxs']");
                var stripsVal = parseInt(strips.val());
                var currentVal = parseInt(input.val());
                var stripinput = $("input[name='box_size']");
                var stripVal = parseInt(stripinput.val());
            ...
            $('.box-plus-minus .input').on('change', function() {
                var minValue = parseInt($(this).attr("min"));
                var maxValue = parseInt($(this).attr("max"));
                var valueCurrent = parseInt($(this).val());
                ...
                name = $(this).attr("name");
                if (valueCurrent >= minValue) {
                    $(this).siblings("[data-type='Bminus']").removeAttr("disabled");
                } else {
                    alert("Sorry, the minimum limit has been reached");
                    $(this).val($(this).data("oldValue"));
                }
                if (valueCurrent <= maxValue) {
                    $(this).siblings("[data-type='Bplus']").removeAttr("disabled");
                } else {
                    alert("Sorry, the maximum limit has been reached");
                    $(this).val($(this).data("oldValue"));
                }
            ...
            // Product page Box quantity Calculation end
        },
